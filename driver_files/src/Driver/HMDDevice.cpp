...
void OpenVREmulatorDriver::HMDDevice::Update()
{
    ...
    this->rot_x_ = std::fmax(this->rot_x_, -kPi / 2.0f);
    this->rot_x_ = std::fmin(this->rot_x_, kPi / 2.0f);

    const XMFLOAT4 yQuatF{0, std::sin(this->rot_y_ / 2), 0, std::cos(this->rot_y_ / 2)};
    const XMFLOAT4 xQuatF{std::sin(this->rot_x_ / 2), 0, 0, std::cos(this->rot_x_ / 2)};
