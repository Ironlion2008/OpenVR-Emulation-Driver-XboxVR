...
vr::EVRInitError OpenVREmulatorDriver::VRDriver::Init(vr::IVRDriverContext *pDriverContext)
{
    ...
    Log("[XboxVR][S1] Driver Init entered");
    ...
    const bool leftAdded = this->AddDevice(std::make_shared<ControllerDevice>(
        "OpenVREmulator_ControllerDevice_Left", ControllerDevice::Handedness::LEFT, config));
    const bool rightAdded = this->AddDevice(std::make_shared<ControllerDevice>(
        "OpenVREmulator_ControllerDevice_Right", ControllerDevice::Handedness::RIGHT, config));
    Log(std::string("[XboxVR][S2] Controller registration: LEFT=") +
        (leftAdded ? "SUCCESS" : "FAILED") + " RIGHT=" +
        (rightAdded ? "SUCCESS" : "FAILED"));
    ...
    Log("[XboxVR] Driver initialization completed; controller Update() will report XInput state once per second");
