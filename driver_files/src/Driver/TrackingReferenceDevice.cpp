...
TrackingReferenceDevice::TrackingReferenceDevice(std::string serial)
    : serial_(std::move(serial)), random_angle_rad_([&] {
          ...
          const float twoPi = 3.14159265358979323846f + 3.14159265358979323846f;
...
void TrackingReferenceDevice::Update()
{
    ...
    const XMFLOAT4 yQuatF{0, std::sin(this->random_angle_rad_ / 2), 0,
                          std::cos(this->random_angle_rad_ / 2)};  // Point inwards (z- is forward)

    const XMFLOAT4 xLookDownF{
        std::sin((-3.14159265358979323846f / 4) / 2), 0, 0,
        std::cos((-3.14159265358979323846f / 4) / 2)};  // Tilt downwards to look at the centre
